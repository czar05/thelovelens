...
import ServerResponseObj from'fwsp-server-response';
...
class ServerResponse {
  ...
  sendInvalidRequest (res, data) {
    data.errorType = data.errorType ? data.errorType : Constants.ERROR_TYPES.GLOBAL_ERROR;
  ...
  /**
   * @name sendTimeOutResponse
   * @summary Send an HTTP_TIMEOUT server response to caller.
   * @param {object} res - Node HTTP response object
   * @param {object} data - An object to send
   * @return {object} res - Returns the (res) response object when in test mode, else undefined
   */
  sendTimeOutResponse (res, data) {
    return this.sendResponse(ServerResponse.HTTP_TIMEOUT, res, data);
  }
}
...
ServerResponse.HTTP_TIMEOUT = 408;
